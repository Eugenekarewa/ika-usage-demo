...
var dwallet_exports = {};
__export(dwallet_exports, {
  EncryptionKeyScheme: () => EncryptionKeyScheme,
  approveAndSign: () => approveAndSign,
  createActiveEncryptionKeysTable: () => createActiveEncryptionKeysTable,
  createEncryptedUserSharesHolder: () => createEncryptedUserSharesHolder,
  getActiveEncryptionKeyObjID: () => getActiveEncryptionKeyObjID,
  getDwalletByObjID: () => getDwalletByObjID,
  getEncryptedUserShare: () => getEncryptedUserShare,
  getEncryptedUserShareByObjectID: () => getEncryptedUserShareByObjectID,
  getEncryptedUserSharesObjID: () => getEncryptedUserSharesObjID,
  getEncryptionKeyByObjectId: () => getEncryptionKeyByObjectId,
  hashToNumber: () => hashToNumber,
  saveEncryptedUserShare: () => saveEncryptedUserShare,
  setActiveEncryptionKey: () => setActiveEncryptionKey,
  storeEncryptionKey: () => storeEncryptionKey,
  transferEncryptedUserShare: () => transferEncryptedUserShare,
  waitForSignOutput: () => waitForSignOutput
});
module.exports = __toCommonJS(dwallet_exports);
...
async function approveAndSign(dwalletCapId, signMessagesId, messages, dwalletID, hash, keypair, client) {
  ...
  if (!(0, import_signature_mpc_wasm.verify_signatures)(
    import_bcs.bcs.vector(import_bcs.bcs.vector(import_bcs.bcs.u8())).serialize(messages).toBytes(),
    hashToNumber(hash),
    new Uint8Array(dwallet?.decentralizedDKGOutput),
    import_bcs.bcs.vector(import_bcs.bcs.vector(import_bcs.bcs.u8())).serialize(signatures).toBytes()
  )) {
...
const storeEncryptionKey = async (encryptionKey, encryptionKeyScheme, keypair, client) => {
  ...
  const tx = new import_builder.TransactionBlock();
  let purePubKey = tx.pure(import_bcs.bcs.vector(import_bcs.bcs.u8()).serialize(encryptionKey));
  let pureSignedPubKey = tx.pure(import_bcs.bcs.vector(import_bcs.bcs.u8()).serialize(signedEncryptionKey));
  let pureSuiPubKey = tx.pure(import_bcs.bcs.vector(import_bcs.bcs.u8()).serialize(keypair.getPublicKey().toRawBytes()));
  tx.moveCall({
    ...
    arguments: [
      ...
      pureSignedPubKey,
      pureSuiPubKey,
      tx.pure(import_bcs.bcs.u8().serialize(encryptionKeyScheme))
    ]
  });
...
const getEncryptedUserSharesObjID = async (client, keypair) => {
  const table = await (0, import_utils.fetchOwnedObjectByType)(
